...
describe("ScoreMarkerOverlay", () => {
  ...
  test("dragging a marker reports its new normalized position and swallows the trailing click", () => {
    const onSelectMarker = jest.fn()
    const onMoveMarker = jest.fn()
    const marker = {
      _id: "m1",
      page: 1,
      frameIndex: 3,
    ...
    fireEvent.mouseDown(pin, { clientX: 40, clientY: 20 })
    fireEvent.mouseMove(window, { clientX: 100, clientY: 50 })
    fireEvent.mouseUp(window, { clientX: 100, clientY: 50 })

    expect(onMoveMarker).toHaveBeenCalledWith(marker, 0.5, 0.5)
    expect(onSelectMarker).not.toHaveBeenCalled()
  })

  test("a plain click without movement still selects the marker for editing", () => {
    const onSelectMarker = jest.fn()
    const onMoveMarker = jest.fn()
    const marker = {
      _id: "m1",
