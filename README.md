...
Elements are slots in which singular pieces of media are stored. On the presentation page, the user can add elements by dragging them to the grid from the media pool panel. Files uploaded to the media pool are stored with the presentation, so the pool is still there after a reload and an entry can be dragged into the grid more than once without being uploaded again. Files can also be directly drag-and-dropped into the grid. Each element in the editor grid has an additional menu through which the user can delete, rename, copy them and toggle the loop state for audio cues. Elements can also be double clicked to quickly rename them. Changes are saved automatically.
